...
def mysql_name(config: Dict) -> str:
    return POLICY_LABEL.get(parse_policy(config), "<unknown>")


def load_mysql_runs(path: str) -> List[bench_lib.BenchRun]:
    runs = bench_lib.parse_results_file(path, bench_lib.BenchResults)
    return [r for r in runs if r.config.get("name") == "mysql_jdbc_benchmark"]
...
def merge_runs(primary: List[bench_lib.BenchRun], extra: List[bench_lib.BenchRun]) -> List[bench_lib.BenchRun]:
    ...
    merged = list(primary)
    for run in extra:
        if not bench_lib.exists_config_in_results(merged, run.config):
            merged.append(run)
    return merged
...
def _has_result_for_benchmark(runs: List[bench_lib.BenchRun], config_match: Dict, benchmark: str) -> bool:
    match = dict(config_match)
    match["benchmark"] = benchmark
    selected = bench_lib.results_select(runs, match)
    return len(selected) > 0


def filter_complete_benchmarks(
    ...
    for bench in benchmarks:
        if all(_has_result_for_benchmark(runs, cfg, bench) for cfg in config_matches):
            valid.append(bench)
    return valid
...
def plot_absolute_figures(
    ...
    group_names = {b: WORKLOAD_LABEL.get(b, b) for b in benchmarks}
...
def summarize_runs(runs: List[bench_lib.BenchRun]) -> Dict[str, Dict[str, Dict[str, float]]]:
    ...
    for benchmark, benchmark_data in grouped.items():
        ...
        # Add policy ranking fields per workload.
        throughput_ranked = sorted(
            summary[benchmark].items(),
            key=lambda item: item[1]["throughput_avg"],
            reverse=True,
        )
        latency_ranked = sorted(
            summary[benchmark].items(),
            key=lambda item: item[1]["latency_p99_ms_avg"],
        )

        for idx, (policy, _) in enumerate(throughput_ranked, start=1):
            summary[benchmark][policy]["throughput_desc_rank"] = idx
        for idx, (policy, _) in enumerate(latency_ranked, start=1):
            summary[benchmark][policy]["latency_p99_asc_rank"] = idx
...
def plot_normalized_figure(
    ...
    for i, policy in enumerate(selected_policies):
        ...
        annotate_bar_values(axes[0], bars_tp)
        annotate_bar_values(axes[1], bars_lat)

    workload_labels = [WORKLOAD_LABEL.get(b, b) for b in valid_benchmarks]
    axes[0].axhline(100, color="black", linestyle="--", linewidth=1)
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(workload_labels, rotation=15)
    axes[0].set_ylabel("Throughput (% of Default)")
    axes[0].set_title("MySQL Throughput (Default = 100%)")

    axes[1].axhline(100, color="black", linestyle="--", linewidth=1)
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(workload_labels, rotation=15)
    axes[1].set_ylabel("P99 Latency (% of Default)")
    axes[1].set_title("MySQL P99 Latency (Default = 100%)")

    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.03), ncol=min(4, len(labels)))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize and plot MySQL benchmark results")
    parser.add_argument(
        "--results-file",
        type=str,
        default="../results/mysql_ycsb_results.json",
        help="Main MySQL benchmark results JSON",
    )
    parser.add_argument(
        "--mglru-results-file",
        type=str,
        default="../results/mysql_ycsb_results_mglru.json",
        help="Optional MGLRU MySQL results JSON",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--benchmarks",
        type=str,
        default="workloada,workloadb,workloadc,workloadd,workloade,workloadf",
...
def main():
    ...
    runs = load_mysql_runs(args.results_file)
    add_config_field_if_missing(runs, "mglru", False)

    if os.path.exists(args.mglru_results_file):
        mglru_runs = load_mysql_runs(args.mglru_results_file)
        add_config_field(mglru_runs, "mglru", True)
        runs = merge_runs(runs, mglru_runs)

    if not runs:
        raise ValueError("No mysql_jdbc_benchmark runs found in input files")

    config_matches, selected_policies, colors = build_config_matches(runs)
    if not config_matches:
        raise ValueError("No recognizable MySQL policy entries found")

    plot_benchmarks = filter_complete_benchmarks(runs, config_matches, benchmarks)
    if not plot_benchmarks:
        raise ValueError(
            "No workload has complete coverage across selected policies; "
            "rerun benchmark with more complete data or narrow --benchmarks"
        )

    skipped = [b for b in benchmarks if b not in plot_benchmarks]
    if skipped:
        print("Skipping workloads with incomplete policy coverage:", ",".join(skipped))
